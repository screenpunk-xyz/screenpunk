...
let state = { scenario:'Partly ready', screen:'TV remote', count:2, view:'menu', guided:'Not set up', appearance:'System', orientation:'Landscape', revision:0 };
...
<aside>First design pass · First launch and the two-finger hold open the same menu. All screens and connections are sample state.</aside>
<iframe title="Native iPad Simulator" src="http://localhost:3200"></iframe>
<script>
let view='menu';
async function update(){const status=document.getElementById('status');status.textContent='Applying…';try{const r=await fetch('/state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scenario:document.getElementById('scenario').value,screen:document.getElementById('screen').value,count:Number(document.getElementById('count').value),guided:document.getElementById('guided').value,appearance:document.getElementById('appearance').value,orientation:document.getElementById('orientation').value,view})});if(!r.ok)throw Error();status.textContent='Applied to preview';}catch{status.textContent='Preview update failed';}}
for(const id of ['scenario','screen','count','guided','appearance','orientation'])document.getElementById(id).addEventListener('change',update);
