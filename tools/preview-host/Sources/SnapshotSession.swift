...
final class SnapshotSession: NSObject, WKNavigationDelegate {
    ...
    func start() {
        ...
        let readyScript = WKUserScript(
            ...
            // Offline previews have no native transport. The SDK preserves this
            // preinstalled runtime, so supply status without advertising controls.
            if (typeof window.screenpunk.runtime.onStatus !== 'function') {
                window.screenpunk.runtime.onStatus = function (listener) {
                    listener({active: false, preview: true, googleTVDirectChannels: 0});
                    return function () {};
                };
            }
...
#if DEBUG
                if ProcessInfo.processInfo.environment["SCREENPUNK_PUBLIC_READ_FIXTURE"] == "1" {
                    webView.evaluateJavaScript("document.body.dataset.loadedFrames === '2' && document.body.dataset.publicReadState === 'stale' && document.querySelector('#frame').naturalWidth > 0 && document.querySelector('#frame').src.startsWith('screenpunk://package/__native-raster/')") { [weak self] result, _ in
                        guard let self else { return }
                        guard (result as? Bool) == true else { self.fail("dynamic_raster_probe"); return }
                        FileHandle.standardError.write(Data("DYNAMIC_RASTER_PROBE_OK\n".utf8))
                        self.capture(webView)
                    }
                    return
                }
