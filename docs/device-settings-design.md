...
### Google TV recovery review — September 24, 2026

Implemented locally, not deployed: Connection port and Pairing port are together under Channels and power, with their source pages explained. Basic address updates and developer endpoint updates verify the existing stored TV pin before saving, retaining permissions and identity. Draft IP/connection-port edits survive reopening and remain distinct from verified saved endpoints. Results appear in the relevant section. Following successful code pairing, failed connection verification retains an in-memory pending pairing and offers Retry connection for the same address; no new code is required within that settings session. Pending pairing is not persisted across app termination. An IP change alone no longer clears developer permissions. Automatic screen access is granted only on successful pairing, not on an ordinary endpoint check/update.

Native SettingsDesign preview mirrors these controls with isolated sample state. Ready shows saved-connection actions; Needs attention shows simulated incomplete pairing and Retry. No network operation occurs in this preview. The standalone simulator browser and studio state controls are open for review. Existing Google TV test suite passed; 14 ADB tests passed after adding endpoint preservation/validation coverage. Native preview compiled and hot reloaded successfully. No physical-device build was deployed, no playback/power commands sent, and changes remain uncommitted.
