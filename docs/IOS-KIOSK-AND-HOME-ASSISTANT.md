...
## Multiple deployed screens

Native devices advertise `screen-set-v1`. The owner-pinned `deploy.set` request contains an ordered set of 1–12 screens, their package files, and optional native Home Assistant provisioning for each screen. The existing 2 MiB encoded frame limit applies to the whole request. A one-screen set replaces any previously installed set.

The receiver verifies all target profiles, package hashes, unique screen/revision membership and Home Assistant bindings before committing. Package directories are immutable; credentials are staged in a Keychain generation. Atomic replacement of `device-state.json` selects the complete package and credential generation together. Preparation failures leave the previous set untouched. Credentials never enter package files or device-state JSON. A repeated deployment ID with identical content returns its original receipt even after the user switches screens; different content with that ID is rejected.

Two-finger horizontal paging follows the fingers and wraps through the deployed order in either direction. Adjacent pages are lightweight named cards with a position label; only the selected dashboard is live. A short two-finger drag settles back without switching. One-finger gestures remain inside the dashboard. Dragging beyond the hold movement tolerance cancels the stationary hold so paging cannot open the menu. Vertical scrolling and the existing zoom lock remain available. No persistent screen indicator covers the dashboard. A five-second stationary two-finger hold opens the device menu; Disconnect opens a separate confirmation before removing screens, owner and credentials. VoiceOver exposes Previous screen, Next screen and Device menu actions. Selection survives relaunch. Only the selected screen can use its exact owner/revision/Keychain grant, and changing selection invalidates pending responses. There is no automatic screen cycling. Existing single-screen installs continue rendering with their owner, package and Home Assistant grant intact until replaced by a screen-set deployment.

Validation includes real local TLS deployment tests for ordered installation, idempotency after selection, relaunch, corrupt later packages, credential-write failure, interrupted preparation, cross-owner/revision grant rejection and replacement with a single screen. The revised carousel and menu are intended for another physical swipe/control check on the iPad; automated coverage checks circular ordering and the five-second hold specification.
