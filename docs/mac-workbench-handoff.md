...
- Start pairing on the Mac, compare the security code, press **Codes Match** on the Mac, and confirm on the phone. The Mac sends `pair.confirm` only after its own confirmation, then polls the authenticated handshake until the device answers; a device that confirms on its own cannot pair itself to the Mac. The device still enforces local user approval.
...
## Multiple screens — September 14, 2026

The Devices header screen selector is a 360-point searchable popover matching the preview-device picker. Screen rows scroll independently of the fixed Duplicate Screen / New Screen footer. Multiple Screens switches between a single choice and an ordered checklist (up to 12). Selecting only changes the Mac preview and pending device selection; Apply Screen / Apply Screens explicitly commits the entire selection. Turning multiple selection off retains the previewed screen when selected, or the first selected screen. An empty selection disables Apply. Search matches all typed terms, ignoring case and diacritics.

Each screen is prepared for the chosen device orientation before transfer. `deploy.set` installs all packages and each screen's native Home Assistant grant atomically; the encoded LAN envelope remains bounded to 2 MiB. A failed preparation, invalid package, or credential staging failure preserves the installed set. A lost connection requires refreshing device status because the commit response might have been lost. The controller persists the installed ordered set and current page, while the Mac retains pending per-device selections when switching sidebar items. Polling follows phone swipes only when there are no unapplied choices. The phone switches screens through left/right two-finger swipes, with no automatic cycling and no dependency on the Mac staying open.

Local validation: all 44 Controller tests pass, including ordered selection, search, limit, failure of the second screen, idempotent retry, changed-payload rejection, persisted set, queried swipe selection, set-to-single replacement, and escaped-envelope size. The Mac arm64 Release build succeeds. The iOS task reports 40 Apple tests and 78 Core tests passing plus a successful iOS Release build. Physical multi-screen swipe testing remains pending; these feature builds have not replaced the previously verified pairing builds.

An isolated Mac UI fixture with 24 sample screens verified live find-as-you-type, Return-to-select and dismiss in single mode, checkbox selection without dismissing in multiple mode, plural Apply Screens label, fixed footer while scrolling, and collapsing back to the previewed single screen. No device transport was attached to the fixture; it was closed after testing.

The native device menu now opens with a stationary two-finger five-second hold. Disconnect is a separate menu action with confirmation; opening the menu never erases a screen. Mac Forget dialogs, second-owner errors, and MCP help/resources all use this same wording. Existing help URI `screenpunk://help/unlink` remains stable. The persistent screen-position pill is removed; all Screenpunk navigation gestures require two fingers, leaving one-finger touches to the dashboard.
